...
describe('Scraper links', function() {
  var testGlobal = {config : require('../lib/test-config').config };
  var sites = [
  ...
  sites.forEach(function(site) {
    ...
    it('should find ' + name + ' nav and results', function() {
      var content = fs.readFileSync('test/data/search-results/' + name + '.html');
      var allLinks = [], links = contentLib.getRecognizedLinks.call({ GLOBAL: testGlobal}, { uri: uri, content: content.toString()});
