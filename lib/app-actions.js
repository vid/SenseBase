...
app.namespace(namespace, function() {
  ...
  app.post('login', passport.authenticate('local', { failureRedirect: namespace + '/login', failureFlash: true }), function(req, res) {
  ...
  app.all('*', function(req, res, next) {
    if (req.user) {
      console.log('go', req.user);
      next();
    } else {
      console.log('stop');
      res.redirect(GLOBAL.config.HOMEPAGE + 'login');
    }
  });

  // all requests past here require auth
...
*/
  ...
  app.post('/upload', function(req, res) {
    ...
    var statusCallback = function(err, res, cItem) {
      if (err) {
       GLOBAL.error('/upload', err);
       } else {
         console.log('uploaded', res, cItem.uri, cItem.content.length);
       }
    };
    fileImport.indexFiles(uploadedFiles, {member: req.user.username, mode: 'upload'}, statusCallback, function(tmpPath) {
