...
// including an asset
  if (request_url.pathname.indexOf('/__wm/') === 0) {
    ...
    GLOBAL.info('asset', asset);
    //FIXME async
    if (asset === 'member.js') {
      response.content = _.template(fs.readFileSync('./web/views/memberjs.html'), { user: foundMember, clientID: foundMember ? foundMember.username + ':' + new Date().getTime() : null, homepage: GLOBAL.config.HOMEPAGE });
    } else {
      try {
