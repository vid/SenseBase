...
exports.setupUsers = function(ctx, users)  {
  ...
  ['Agent', 'Searcher'].forEach(function(type) {
    _.forEach(_.where(users, { status: 'available', type: type}), function(a) { ctx.authed[a.username] = a; });
  });
  context = ctx;
};
...
exports.memberJS = function(member) {
  member = member || {};
  return memberTemplate({ localJS: context.config.localJS, user: member.username, clientID:  member.clientID, homepage: context.config.HOMEPAGE });
};
