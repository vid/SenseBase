...
var DEFAULT_USERS_LOC = '../local-site.json', memberTemplate =  _.template(fs.readFileSync(path.join(process.env.PWD, './web/site/member.js')));
...
exports.setupUsers = function(ctx, users)  {
  ...
  _.forEach(_.where(users, { status: 'available'}), function(a) { ctx.authed[a.username] = a; });
  context = ctx;
};
...
exports.memberJS = function(member) {
  member = member || {};
  return memberTemplate({ user: member.username, clientID:  member.clientID, homepage: context.config.HOMEPAGE });
};
