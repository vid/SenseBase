...
function getQueuedLink(callback, seconds) {
 ...
 // call query with the function
  GLOBAL.svc.indexer.formQuery({query: { terms: dateField + ':<now-' + seconds + 's AND state:queued AND queued.attempts:<' + MAX_ATTEMPTS}}, process,
    { sort: dateField, sourceFields : ['uri', 'title', 'queued.*'], size : 1 });
}
...
function queueSearcher(data) {
  ...
  if (data.team && data.team.length > 0 && data.input && data.member && data.categories && data.categories.length > 0) {
    ...
    // log the search
    GLOBAL.svc.indexer.saveSearchLog({searchID: GLOBAL.svc.indexer.searchID(data), searchDate: new Date()}, function(err, res) {
      utils.passingError(err);
    });
    var input = encodeURIComponent(data.input);
    // process it by each member
    data.team.forEach(function(m) {
      var user = auth.getUserByUsername(m), context = { member: user.username, input: input, relevance: data.relevance, team: data.team, categories: data.categories};
      // user not found
      if (!user) {
        ...
        if (user.type === 'Searcher') {
          if (user.api) {
            ...
            context.query = (user.template || '$SBQUERY').replace('$SBQUERY', input);
