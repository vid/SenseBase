...
function queueLink(uri, context) {
  indexer.retrieveByURI(uri, function(err, result) {
    if (!result) {
      ...
      var cItem = annotations.createContentItem({ title: 'Queued ' + context.categories, uri: uri, referers: [context.referers], state: utils.states.queued, queued: queuedDetails});
...
function getLink(err, queuedLink) {
  ...
  method(queuedLink.uri, function(err, data) {
    contentLib.indexContentItem({ uri: queuedLink.uri, content: data, member: queuedLink.queued.member}, function(err, res) {
      utils.passingError(err);
