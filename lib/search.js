...
function queueSearcher(data) {
  // validate
    // FIXME move validation to client-server lib
  if (data.team && data.team.length > 0 && data.input && data.member && data.categories && data.categories.length > 0) {
    // Update last run of search
    if (data.lastSearch) {
      data.searches.push(data.lastSearch);
    } else {
      data.searches = [];
    }
    data.lastSearch = new Date();
    GLOBAL.config.indexer.saveSearch(data, function(err, res) {
      if (err) {
        throw (err);
      }
