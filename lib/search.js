...
function queueLink(uri, context) {
  indexer.retrieveByURI(uri, function(err, result) {
    if (!result) {
      ...
      var cItem = annotations.createContentItem({ title: 'Queued ' + context.categories, uri: uri, state: utils.states.content.queued, queued: queuedDetails});
      contentLib.indexContentItem(cItem, context, function(err, res, cItem) {
      });
    } else {
...
function getLinkContents(err, queuedLink) {
  ...
  method(queuedLink.uri, function(err, data) {
    contentLib.indexContentItem(annotations.createContentItem({ uri: ''+queuedLink.uri, content: data}), {member: queuedLink.queued.member}, function(err, res, cItem) {
      if (err || !cItem) {
        GLOBAL.error('getLinkContents failed', err);
        return;
      }
