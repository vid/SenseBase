...
exports.states = { validated : 'validated', unvalidated : 'unvalidated', erased : 'erased', queued: 'queued'};

// retrieve the named member from global member list
exports.getMemberByUsername = function(m) {
  for (var i = 0; i < GLOBAL.config.users.length; i++) {
    if (GLOBAL.config.users[i].username === m) {
      return GLOBAL.config.users[i];
    }
  }
}
