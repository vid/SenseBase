...
console.log('processing with', useServices.map(function(m) { return m.name}));
  var getAnnos = queue();
  useServices.forEach(function(service) { // get cached or new for doc
    getAnnos.defer(function(saveCB) {
      if (savedAnnos[service.name][uri]) {
        ...
        try {
          ...
          console.log('annotate ERROR', service.name, e);
          saveCB(e);
        }
      }
    });
    getAnnos.awaitAll(function(err, results) {
      var all = [];
      if (results) {
        require('fs').writeFile('/tmp/results', JSON.stringify(results, null, 2));
