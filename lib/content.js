...
var BASE_FIELDS = ['annotations', 'visitors', 'referers'];
...
// it has been indexed
    ...
    console.log(user.type, desc.title, 'v', validated.extracted);
    if (user.type === 'User') {
    ...
    if (hItem) {
      if (context.deleteExistingAnnotatedBy) {
        for (var i = 0; i < hItem.annotations.length; i++) {
          if (hItem.annotations[i].annotatedBy === context.deleteExistingAnnotatedBy) {
            delete hItem.annotations[i];
          }
        }
      }
    ...
    var diffs = diffContentItems(hItem, cItem);
    if (diffs.length > 0) {
      GLOBAL.debug('UPDATING', diffs);
      ...
      GLOBAL.debug('NOT UPDATED', diffs.length, diffs);
...
function diffContentItems(lhs, rhs) {
  ...
  //field values
  ['title', 'content', 'sha1', 'created', 'state', 'uri', 'text'].forEach(function(i) {
    if (lhs[i] !== rhs[i]) {
      diffs.push({ field: i, diff: 'different', lhs: lhs[i], rhs: rhs[i]});
    }
  });

  ['headers', 'queued'].forEach(function(i) {
    if (!_.isEqual(lhs[i], rhs[i])) {
      diffs.push({field: i, diff: 'different', lhs: lhs[i], rhs: rhs[i]});
    }
  });

  // array values
  ['visitors', 'referers', 'annotations'].forEach(function(i) {
    if (lhs[i]) {
      lhs[i].forEach(function(li) {
        if (!rhs[i] || !_.pluck(rhs[i], li)) {
          diffs.push({ field: i, diff: 'missing-rhs', lhs: li });
        }
      });
    }

    if (rhs[i]) {
      rhs[i].forEach(function(ri) {
        if (!lhs[i] || !_.pluck(lhs[i], ri)) {
          diffs.push({ field: i, diff: 'missing-lhs', rhs: ri });
        }
      });
    }
