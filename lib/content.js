...
function replaceThenMergeAnnotations(annos, nannos) {
  var nanno, n, a, anno;
  for (a = 0; a < annos.length; a++) {
    anno = annos[a];
    for (n = nannos.length - 1; n > -1; n--) {
      nanno = nannos[n];
      console.log(anno.flattened, 'vs', nanno.flattened);
      if (anno.flattened === nanno.flattened) {
        var nuser = GLOBAL.svc.auth.getUserByUsername(nanno.annotatedBy);
        console.log('HUH', nuser, nanno.annotatedBy);
        if (!nuser.needsValidation) {
          console.log('replacing', anno, 'with', nanno);
          annos[a] = nanno;
        }
        nannos.splice(n, 1);
      }
