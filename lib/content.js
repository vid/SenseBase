...
// context: { member: psMember, referer: referer, isHTML: browser_request.is_html, annotators: [], categories: [],
// queueOnly: don't index if it already exists }
function indexContentItem(desc, context, callback) {
  console.log('receiving data', desc.uri, desc.content ? desc.content.length : 0);
  context = context || {};
  if (!context.member) {
...
// it has been indexed
    ...
    if (context.categories) {
      context.categories.forEach(function(category) {
        ...
        desc.annotations.push(annotations.createAnnotation({hasTarget: desc.uri, type: 'category', category: category, state: desc.state, annotatedBy: context.member}));
      });
    }
