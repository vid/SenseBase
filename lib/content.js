...
// find any existing instance
  GLOBAL.svc.indexer.retrieveByURI(desc.uri, function(err, result) {
    var cItem, hItem;
// it has been indexed
    if (result && result._source) {
      ...
      if (!desc.created) {
        ...
        desc.title = desc.title || hItem.title;
      }
    }
    ...
    cItem.annotations = cItem.annotations.concat(annotationsFromCategories(cItem, context));
    ...
    GLOBAL.svc.indexer.saveContentItem(cItem, function(err, res, cItem) {
      if (err || !cItem) {
        ...
        if (!context.updating && cItem.state !== utils.states.content.queued) {
          ...
          pubsub.init({ homepage: GLOBAL.config.HOMEPAGE, clientID: clientID }).itemIndexed(cItem, desc);
        }
...
// extract recognizable content
    ...
    ret.title = extracted.title.trim() || desc.title;
    if (!ret.title) {
...
function annotationsFromCategories(cItem, context) {
  var ret = [];
  if (context.categories) {
    context.categories.forEach(function(category) {
      //FIXME annotation state
      ret.push(annotations.createAnnotation({hasTarget: cItem.uri, type: 'category', category: category, state: utils.states.annotations.validated, annotatedBy: context.member}));
    });
  }
  return ret;
}
