...
// apply validation state based on user
  ...
  fayeClient.subscribe('/search', function(data) {
    ...
    GLOBAL.config.indexer.formSearch(data, function(err, results) {
      if (err) {
        console.log('/search query failed', JSON.stringify(results.query, null, 2));
      }
      if (results.hits) {
        GLOBAL.info('searchResults', results.hits.hits.length);
  ...
  fayeClient.subscribe('/cluster', function(data) {
    GLOBAL.info('/cluster', data);
    GLOBAL.config.indexer.formCluster(data, function(err, results) {
      if (results) {
        GLOBAL.info('clusterResults', err, results.hits.hits.length);
...
// process incoming scrapes and send queued links
  fayeClient.subscribe('/visited', function(site) {
    var scraped = site.scraped, links = site.links || [], tags = site.tags || 'scraper-no-tag', scraper = site.scraper, validated = true;

    if (scraped) {
        var anno = annotations.createAnnotation({hasTarget: scraped, type: 'category', category: tags, validated: validated, annotatedBy: scraper});
        indexer.saveAnnotations(scraped, [anno], utils.passingError);
    }
    if (links) {
      links.forEach(function(uri) {
        var cItem = annotations.createContentItem({ title: 'Queued ' + tags, uri: uri, referers: [scraped], state: 'queued', queued: { tags: tags}});
...
// print info when we have number of hits
      GLOBAL.info('/visited', site, res.hits);
      if (res.hits && res.hits.length > 0) {
        // FIXME add timing for multiple clients
        var next = results.hits.hits[0]._source;
...
function requestAnnotate(uri, html) {
  contentExtractor.extractContent(uri, html, function(uri, html, selector, content) {
    console.log('selector', selector, content ? content.length : 'nocontent');
