...
// FIXME change the state of an annotation { uri: uri, annotations: [annotations] }
  clientSubscribe('/item/annotations/changestate', function(combo, clientID, username) {
    GLOBAL.info('/item/annotations/changestate'.yellow, combo);
    changeAnnotationState(combo, username);
  });
...
// retrieve annotations for an individual uri
  ...
  clientSubscribe('/query/request', function(options, clientID) {
    ...
    query(options, function(err, results) {
      ...
      if (results.hits) {
        ...
        publish('/query/results', {results: results, options: options}, clientID);
        if (nav) {
  ...
  clientSubscribe('/item/moreLikeThis', function(options, clientID) {
    GLOBAL.info('/item/moreLikeThis'.yellow, options);
    GLOBAL.svc.indexer.moreLikeThis(options.uri, function(err, results) {
      GLOBAL.info('/query/results'.bgYellow, (results || []).length);
      if (results) {
        publish('/query/results', {results : results, options: options}, clientID);
      } else {
        publish('/query/results', {results: [], options: options}, clientID);
  ...
  clientSubscribe('/query/moreLikeThisContent', function(options, clientID) {
    GLOBAL.info('/query/moreLikeThisContent'.yellow, options);
    try {
      GLOBAL.svc.indexer.moreLikeThisContent(options, function(err, results) {
        GLOBAL.info('/query/results'.bgYellow, (results || []).length);
...
// queue up a search
  clientSubscribe('/search/queue', function(data, clientID, username) {
    ...
    searchLib.queueSearcher(data, function(err, status) {
      publish('/status/queued', { err: err, status: status}, clientID);
    });
  });
...
// FIXME change the state of annotations
  function changeAnnotationState(combo, username) {
...
function publishRetrievedSearches(username) {
  GLOBAL.svc.indexer.retrieveSearches({}, function(err, results) {
