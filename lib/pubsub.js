...
// user functions
  ...
  fayeClient.subscribe('/team/list', function(message) {
...
// apply validation state based on user
    ...
    fayeClient.publish('/annotations', combo.annotations);
  });
...
// annotations for an individual uri
  fayeClient.subscribe('/annotate', function(message) {
    ...
    indexer.retrieveAnnotations(message.uri, function(err, results) {
      fayeClient.publish('/annotations', results.hits.hits.map(function(h) { return h._source}));
    });
  });

  fayeClient.subscribe('/delete', function(cItemURIs) {
    GLOBAL.info('/delete', cItemURIs);
    indexer.deleteContentItems(cItemURIs.selected);
  });
