...
// annotations
  ...
  fayeClient.subscribe('/saveAnnotations', function(combo) {
    GLOBAL.info('saveAnnotations', combo.annotations.length);
// apply validation state based on user
    ...
    GLOBAL.config.indexer.saveAnnotations(combo.annotations);
...
// retrieve and send annotations
        ...
        indexer.retrieveAnnotations(uris, function(err, results) {
          fayeClient.publish('/annotations', results.hits.hits);
        });
      } else {
