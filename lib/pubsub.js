  ...
  fayeClient.subscribe('/annotate', function(data) {
    ...
    indexer.retrieveByURI(data.uri.replace(/#.*$/, ''), function(err, res) {
      if (err) {
        ...
        fayeClient.publish('/annotations/' + data.clientID, { uri: data.uri, annotationSummary: res._source.annotationSummary, annotations: res._source.annotations});
      }
    });
  });
