...
// process incoming links
  fayeClient.subscribe('/links', function(site) {
    ...
    var scraped = site.scraped, links = site.links || [], tags = site.tags || 'scraper-no-tag', scraper = site.scraper, validated = false;
    ...
    if (links) {
      links.forEach(function(uri) {
        var cItem = annotations.createContentItem({ scrapeID: tags, title: 'Queued ' + tags, uri: uri, referers: [scraped], state: 'queued'});
...
// send out a new link if available
    indexer.formSearch({validationState : 'queued'}, function(err, res) {
      if (res.hits.length > 0) {
        ...
        fayeClient.publish('/scrape', { site: { uri: next.uri }});
      }
    });
  });
