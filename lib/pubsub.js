    ...
    if (links && relevance > 0) {
      links.forEach(function(uri) {
        ...
        if (uri.indexOf('http:') === 0) {
          uri = uri.replace(/#.*/, '');
          if (!visitingLinks[uri]) {
            var queuedDetails = { tags: tags, relevance: relevance, lastAttempt: new Date().getTime()}
...
// print info when we have number of hits
      GLOBAL.info('/visited'.yellow.yellow, site.uri, res.hits.total);
...
console.log('checking found', next.uri, visitingLinks[next.uri]);
          if (visitingLinks[next.uri]) {
            var visiting = visitingLinks[next.uri];
            // wait at least ten seconds between each URI attempt
            if (visiting.lastAttempt < (new Date().getTime + 20000)) {
              console.log('found', visiting);
