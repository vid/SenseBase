...
// process incoming links
  fayeClient.subscribe('/links', function(site) {
    ...
    var scraped = site.scraped, links = site.links, tags = site.tags, scraper = site.scraper;

// remove scraped link
    if (scraped) {
      var saveError = function(err, res) { if (err) { GLOBAL.error('scrape anno failed', err); } } ;
      for (var link in scrapeLinks) {
        if (scrapeLinks[link].link === scraped) {
          var anno = annotations.createAnnotation({hasTarget: link, type: 'category', category: tags, validated: true, annotatedBy: scraper});
          indexer.saveAnnotations(link, [anno], saveError);

          seenLinks[link] = new Date().getTime();
          delete scrapedLinks[link];
        }
      }

      links.forEach(function(link) {
        if (link && !seenLinks[link]) {
          scrapeLinks.push({ link : link, tags: tags, scraper: scraper });
        }
      });
      if (scrapeLinks.length) {
        GLOBAL.info('pushing scrape', scrapeLinks[0]);
        fayeClient.publish('/scrape', scrapeLinks[0]);
        scrapeLinks.push(scrapeLinks.splice(0, 1));
      }
      console.log('scrapeLinks', scrapeLinks);
    }
  });
