...
// Annotations functions
  ...
  // batch updates per a size or time frame. note bounceHold may be exceed due to the delay.
  function debounceUpdate(combo) {
      bouncedUpdates.push(combo);
      if (bouncedUpdates.length < bounceHold) {
        clearTimeout(bounceTimeout);
        bounceTimeout = setTimeout(function() {
          var toDebounce = bouncedUpdates.slice(0);
          console.log('ssending', toDebounce.length);
          bouncedUpdates = [];
