...
// annotations for an individual uri
  fayeClient.subscribe('/annotate', function(data) {
    ...
    indexer.retrieveAnnotations(data.uri.replace(/#.*$/, ''), function(err, annotations) {
      if (err) {
        ...
        fayeClient.publish('/annotations', { uri: data.uri, annotations: annotations});
      }
    });
  });
  ...
  // apply validation state based on user
      ...
      GLOBAL.config.indexer.saveAnnotations(uri, combo.annotations);
      fayeClient.publish('/annotations', { uri: uri, annotations: combo.annotations});
    });
  });
