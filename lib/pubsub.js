...
// retrieve annotations for an individual uri
  ...
  clientSubscribe('/saveAnnotations', function(combo, clientID, username) {
    ...
    var user = utils.getMemberByUsername(username);
    try {
  ...
  clientSubscribe('/subscriptions/retrieve', function(combo, clientID, username) {
    ...
    indexer.retrieveSubscriptions(username, function(err, results) {
...
function publishRetrievedSearches(username) {
  indexer.retrieveSearches({member: username}, function(err, results) {
