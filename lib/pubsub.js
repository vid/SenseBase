...
// retrieve annotations for an individual uri
  fayeClient.subscribe('/annotate', function(data) {
    ...
    indexer.retrieveAnnotations(data.uri.replace(/#.*$/, ''), function(err, annotations) {
      if (err) {
        GLOBAL.error('/annotate error' + err);
      } else {
        fayeClient.publish('/annotations/' + data.clientID, { uri: data.uri, annotations: annotations});
      }
    });
  });
  ...
  fayeClient.subscribe('/saveAnnotations', function(combo) {
    ...
    // apply validation state based on user
        combo.annotations.forEach(function(p) {
          p.annotatedBy = combo.annotatedBy;
          p.hasTarget = uri;
        ...
        GLOBAL.config.indexer.saveAnnotations(uri, annos, function(err, res, cItem) {
          console.log('OO', '/annotations/' + combo.clientID);
          fayeClient.publish('/annotations/' + combo.clientID, { uri: uri, annotationSummary: cItem.annotationSummary, annotations: JSON.stringify(cItem.annotations)});
        });
      });
    } catch (err) {
