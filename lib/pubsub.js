...
// annotations for an individual uri
  ...
  fayeClient.subscribe('/saveAnnotations', function(combo) {
    GLOBAL.info('saveAnnotations', (combo.annotator || 'unknown').yellow, combo.uri, combo.annotations.length);
      ...
      data.team.forEach(function(m) {
        ...
        if (!member) {
          ...
          member.locations.split('\n').forEach(function(l) {
            ...
            scraperLib.queueLink(l, data);
          });
          // an annotator or individual
          } else {
            console.log(' /queueSearch', l);
            scraperLib.queueLink(data.uri, data);
          }
        }
      });
    } else {
