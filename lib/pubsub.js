...
function clientSubscribe(loc, cb) {
  fayeClient.subscribe(loc, function(data) {
    var user = auth.userByClientID(data.clientID);
    if (user) {
      cb(data, data.clientID, user.username);
    } else {
...
// retrieve annotations for an individual uri
  ...
  // index subscriptions
  clientSubscribe('/saveSubscriptions', function(combo, clientID, member) {
    GLOBAL.info('/saveSubscriptions'.yellow, combo);
  ...
  // index subscriptions
  fayeClient.subscribe('/subscriptions', function(combo, clientID, member) {
    GLOBAL.info('/subscriptions'.yellow, combo);
    combo.matches.forEach(function(match) {
      var sub = {member: member, match: match.trim(), created: new Date()};
      GLOBAL.config.indexer.saveSubscription(sub);
    });
...
// find existing searches
  clientSubscribe('/search/retrieve', function(data) {
    GLOBAL.info('/search/retrieve'.yellow, data);
    publishRetrievedSearches();
  });

// save a search
  clientSubscribe('/search/save', function(search) {
...
// remove a search
  clientSubscribe('/search/remove', function(data) {
    GLOBAL.info('/search/remove'.yellow, data);
  });

// subscribe to query requests
  clientSubscribe('/search/queue', function(data) {
