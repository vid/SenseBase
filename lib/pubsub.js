...
// find existing searches
  fayeClient.subscribe('/search/list', function(data) {
    GLOBAL.info('/search/list'.yellow, data);
    fayeClient.publish('/search/results', { searches: [] });
  });

// save a search
  fayeClient.subscribe('/search/save', function(data) {
    GLOBAL.info('/search/save'.yellow, data);
  });
