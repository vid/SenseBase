...
var utils = require('./utils'), annotations = require('./annotations.js'), contentLib = require('./content'), searchLib = require('./search.js');
...
// Desc contains additional content data.
  clientSubscribe('/itemIndexed', function(combo) {
...
// retrieve annotations for an individual uri
  ...
  clientSubscribe('/saveAnnotations', function(combo, clientID, username) {
    GLOBAL.info('saveAnnotations'.yellow, username.blue, combo.uris, combo.annotations.length);
  ...
  clientSubscribe('/subscriptions/save', function(combo, clientID, username) {
    ...
    combo.matches.forEach(function(match) {
      ...
      GLOBAL.svc.indexer.saveSubscription(sub);
    });
  });

  // index subscriptions
  clientSubscribe('/subscriptions/retrieve', function(combo, clientID, username) {
  ...
  clientSubscribe('/cluster', function(data, clientID) {
    ...
    GLOBAL.svc.indexer.formCluster(data, function(err, results) {
      if (results) {
  ...
  clientSubscribe('/updateContent', function(desc, clientID, username) {
...
// find existing searches
  clientSubscribe('/search/retrieve', function(data, clientID, username) {
