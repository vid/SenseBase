...
var _ = require('lodash'), elasticsearch = require('es'), es, http = require('http'),  crypto = require('crypto');
...
exports.saveSubscription = saveMemberRecord('subscriptions', 'match');
exports.retrieveSubscriptions = retrieveMemberRecords('subscriptions', ['member', 'match', 'created']);

exports.saveSearch = saveMemberRecord('savedSearches', 'searchName');
exports.retrieveSearches = retrieveMemberRecords('savedSearches', ['searchName', 'cron', 'input', 'relevance', 'team', 'categories', 'member', 'targetResults', 'lastSearch', 'searches']);

exports.saveQuery = saveMemberRecord('savedQueries', 'queryName');
exports.retrieveQueries = retrieveMemberRecords('savedQueries', ['queryName']);

// for testing
exports.saveMemberRecord = saveMemberRecord;
exports.retrieveMemberRecords = retrieveMemberRecords;
...
function saveMemberRecord(rType, idField) {
  return function(record, callback) {
    if (!record.member) {
      callback(utils.passingError('no member', record));
    } else {
      indexBulk({_index : GLOBAL.config.ESEARCH._index, _type : rType, _id: record.member + '/' + encodeURIComponent(record[idField])}, record, function(err, res) {
        callback(err, res);
      });
    }
  };
...
// Retrieve member record partial
// FIXME limit "for"
function retrieveMemberRecords(rType, source) {
