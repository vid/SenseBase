...
function _saveContentItem(cItem, callback) {
  ...
  if (!cItem.visitors) {
    ...
    cItem.state = 'found';
    if (cItem.scrapeID) {
      delete cItem.scrapeID;
      console.log('CCC', cItem.visitors);
      var anno = annotations.createAnnotation({hasTarget: cItem.uri, type: 'category', category: cItem.scrapeID, validated: 'T', annotatedBy: cItem.visitors[0].member});
