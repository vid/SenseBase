...
exports.updateAnnotationSummary = updateAnnotationSummary;
    ...
    retrieveByURI(uri, function(err, uriRes) {
      ...
      getEs().index({_type: 'removedItem'}, uriRes._source, function (err, removeRes) {
        if (err) { console.log('delete anno err', err); return; }
          retrieveAnnotations(uri, function(err, annos) {
            console.log(annos);
            console.log('FIXME save deleted annos');
            if (annos.hits.total > 0) {
              annos.hits.hits.forEach(function(anno) {
                //FIXME save annotations to removed
                getEs().delete({ _type: 'annotation', _id : anno._id}, function(err, deleteAnno) {
                });
              });
            }
          });
          //FIXME callback after anno delete
