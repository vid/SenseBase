...
// annomerge
  if (search.annotationSearch) { 
    if (search.annotationSearch.indexOf(':') > 0) {
      // FIXME value hack
      var a = search.annotationSearch.replace(/"/g, '').split(':');
      qs.push('annotations.key:' + a[0] + ' AND annotations.value:' + a[1]);
    } else {
      qs.push('annotations.category:' + search.annotationSearch);
    }
  }
