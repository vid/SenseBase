...
function deleteContentItems(cItemURIs, callback) {
  console.log('CC',cItemURIs);
  cItemURIs.forEach(function(uri) {
    retrieveByURI(uri, function(err, res) {
      if (err) { console.log('delete retrieve err'); return; }
      getEs().index({_type: 'delete removedItem'}, res._source, function (err, res) {
        if (err) { console.log('delete index err'); return; }
        retrieveAnnotations(uri, function(err, res) {  
          if (err) { console.log('delete anno err'); return; }
          //FIXME
...
var query = '*';
  ...
  if (search.terms) { 
    if (search.terms.indexOf(':') < 0) {
      query = 'content:"' + search.terms + '" OR title:"' + search.terms + '"';
    } else {
      query = search.terms;
    }
  }
