...
function updateContent(target, content, callback) {
  // FIXME: set proper state
  getEs().update({_type: 'contentItem', _id: encodeURIComponent(target)}, { "doc" : { content: content, state: 'visited'} }, function(err, res) {
    if (err) {
...
function formCluster(search, callback) {
  ...
  utils.doPost('/contentItem/_search_with_clusters', JSON.stringify(q), function(err, res) {
    res.query = q;
    if (!res || res.hits.hits.length < 1) {
      GLOBAL.info('NO HITS FOR', JSON.stringify(q, null, 2));
    }
...
function moreLikeThis(uri, callback) {
  retrieveByURI(uri, function(err, res) {
    ...
    utils.doPost('/contentItem/_search', data, callback);
  });
}
