...
function retrieveSearches(search, callback) {
  var qchecks = [], qhosts = [];
  var q = {
    _source : ['name', 'cron', 'input', 'relevance', 'team', 'tags', 'member', 'targetResults'],
    size : hitSize,
    "query": {
      "filtered": {
        "query": {
  ...
  if (params.validationState && params.validationState !== 'all') {
    ...
    switch(params.validationState) {
      case 'queued' :
        ...
        break;
      case utils.states.erased :
        qs.push('state:' + utils.states.erased);
        break;
      case 'valOrUnval' :
...
// Executes a form params, returning results
