...
function retrieveSubscriptions(member, callback) {
  var qchecks = [], qhosts = [];
  var q = {
    _source : ['member', 'match', 'creaeted'],
    size : hitSize,
    "query": {
      "filtered": {
        "query": {
          "bool": {
            "must": [{ match : { member : member } }]
          }
        }
      }
    }
  ...
  getEs().search({ _index : GLOBAL.config.ESEARCH._index, _type : 'savedSubscriptions'}, q, function(err, res) {
    callback(err, res);
  });
}
...
// retrieve existing searches
function retrieveSearches(search, callback) {
  var qchecks = [], qhosts = [];
  var q = {
