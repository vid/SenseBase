...
// reformats a doc to annotation format
function esDocToAnnoDoc(esDoc) {
  var annoDoc = {fields : {}};
...
// formats a annotationItem based on input
...
// indexes a annotationItem by inserting or updating, returning it in publishFields format
...
// check if it has already been indexed
  retrieveByURI(options.uri, function(err, results) {
    ...
    if (esDoc) {
      getEs().index({_type: 'annotationItem', _id: encodeURIComponent(options.uri)}, esDoc, function (err, res) {
        if (err) {
          GLOBAL.error('INDEXING error', options.uri, err);
        }
        if (options.callback) {
          options.callback(err, res, esDocToAnnoDoc(esDoc));
        }
      });
    }
  });
...
function formSearch(search, callback) {
  ...
  getEs().search({ _index : 'ps', _type : 'annotationItem'}, q, function(err, res) {
