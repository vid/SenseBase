...
function getEsDoc(options) { //uri, title, psMember, referer, isHTML, content, contentType, headers, annotations) {
  var esDoc;
  esDoc = { uri : options.uri, title: options.title, content : options.content, '@timestamp': new Date().toISOString(), visitors: [], referers: []};

  if (options.psMember) {
    esDoc.visitors.push({ member: options.psMember, '@timestamp': new Date().toISOString() });
  }
  if (options.referer) {
...
function indexPage(options) {
 // uri, title, psMember, referer, isHTML, content, contentType, headers, annotations, callback) {
  if (!options.isHTML) {
