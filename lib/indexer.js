...
exports.saveSubscription = saveRecord('subscriptions', function(data) { return encodeURI(data.member + '/' + data.match); });
exports.retrieveSubscriptions = retrieveRecords('subscriptions', ['member', 'match', 'created']);

// reusabel ID generation
...
function saveRecord(rType, idFun) {
  ...
  return function(record, callback) {
    if (idFun) {
      meta._id  = idFun(record);
    }
    indexBulk(meta, record, function(err, res) {
      callback(err, res);
    });
  };
}
