...
function _saveContentItem(cItem, callback) {
  ...
  if (!cItem.visitors) {
    ...
    cItem.state = 'visited';
    if (cItem.queueTag) {
      delete cItem.queueTag;
      console.log('CCC', cItem.visitors);
      var anno = annotations.createAnnotation({hasTarget: cItem.uri, type: 'category', category: cItem.queueTag, validated: 'T', annotatedBy: cItem.visitors[0].member});
...
// FIXME delay for indexing
      setTimeout(function() { updateAnnotationSummary(target, callback); }, 1000);
    }
...
function updateContent(target, content, callback) {
  getEs().update({_type: 'contentItem', _id: encodeURIComponent(target)}, { "doc" : { content: content} }, function(err, res) {
    if (err) {
      GLOBAL.error('updateContent update', err);
    }
...
function updateAnnotationSummary(target, callback) {
  ...
  getEs().search({ _index : GLOBAL.config.ESEARCH._index, _type : 'annotation'}, query, function(err, res) { // TODO add annotatedBy
    ...
    if (err) {
      ...
      getEs().update({_type: 'contentItem', _id: encodeURIComponent(target)}, { "doc" : { state: 'annotated', annotationSummary: annotationSummary} }, function(err, res) { 
        if (err) {
...
// Direct post for unsupported ES api calls
