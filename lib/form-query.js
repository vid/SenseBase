...
exports.filter = function(filters, res) {
  ...
  _.pluck(res.hits.hits, '_source').forEach(function(cItem) {
    cItem.annotations.forEach(function(anno) {
       if (!shouldKeep(filters, anno)) {
         console.log('removing', anno);
         delete cItem.annotations[anno];
       }
    });
  });
  return res;
...
function shouldKeep(filters, anno) {
  filters.forEach(function(filter) {
    if (filter.type === anno.type) {
      if (_.isEqual(filter.position, anno.position)) {
        if (anno.isA && anno.isA == 'Date') {
          var filterDate = new Date(filter.value), fieldDate = new Date(anno.value);
          if (filter.operator === '<') {
...
exports.summarizeAnnotations = function(results) {
...
exports.createFormQuery = function(params) {
  var qs = [], query_string;
