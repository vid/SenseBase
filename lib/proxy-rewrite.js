...
var utils = require('./utils.js'), contentLib = require('./content.js');
...
exports.onRetrieve = function(uri, referer, is_html, pageBuffer, contentType, saveHeaders, browser_request) {
  ...
  contentLib.indexContentItem(desc, { categories: ['proxy'], member: psMember});
};
...
exports.onRequest = function(browser_request, browser_response) {
  ...
  if (request_url.pathname.indexOf('/__wm/') === 0) {
    ...
    //FIXME async
    if (asset === 'member.js') {
      response.content = _.template(fs.readFileSync('./web/site/member.js'), { user: foundMember, clientID: foundMember ? foundMember.username + ':' + new Date().getTime() : null, homepage: GLOBAL.config.HOMEPAGE });
