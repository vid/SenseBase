        ...
        parser.parseString(prexml.pre, function (err, obj) {
          ...
          ['DateCreated', 'DateCompleted', 'Daterevised'].forEach(function(field) {
            ...
            try {
              ...
              annoRows.push(annotations.createAnnotation({type: 'value', annotatedBy: name, hasTarget: uri, key: field, value: date, attributes: attributes}));
            } catch (e) {
              GLOBAL.debug('fail', e);
            }
          });
...
//            console.log('A', JSON.stringify(mlCitation.Article, null, 2));
            ...
            ['ArticleDate'].forEach(function(field) {
              ...
              try {
                var date = getMeshDate(mlCitation[field]);
                annoRows.push(annotations.createAnnotation({type: 'value', annotatedBy: name, hasTarget: uri, key: field, roots: ['Article'], value: date, attributes: attributes}));
              } catch (e) {
                GLOBAL.debug('fail', e);
              }
            });
