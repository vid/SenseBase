  ...
  if (relevance > 0) {
    ...
    var typeLinks = links[type], referers = [cItem.uri], validated = true, scraper = GLOBAL.config.project
      ...
      typeLinks.forEach(function(uri) {
        queueLink(uri, { relevance: relevance, tags: tags, referers: referers, member: cItem.queued.member, validated: validated});
      });
    }
    return links;
  }
  ...
  indexer.retrieveByURI(uri, function(err, result) {
    if (!result) {
      ...
      context.tags.forEach(function(tag) {
        cItem.annotations.push(annotations.createAnnotation({hasTarget: cItem.uri, type: 'category', category: tag, validated: context.validated, annotatedBy: context.member}));
      });
