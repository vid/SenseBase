...
function queueLinks(cItem) {
  ...
  if (relevance > 0) {
    ...
    var typeLinks = links[type], referers = [cItem.uri], state = utils.states.validated, scraper = GLOBAL.config.project
      // FIXME add referers
...
function getQueuedLink(callback, seconds) {
 ...
 // call search with the function
  indexer.querySearch({validationState : 'queued', terms: dateField + ':<now-' + seconds + 's AND queued.attempts:<' + MAX_ATTEMPTS}, process, 
