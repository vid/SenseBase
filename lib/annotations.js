...
function createContentItem(desc) {
  ...
  var cItem = { title: desc.title, uri: desc.uri, headers: desc.headers || {}};
  // save values as arrays
  ['annotations', 'visitors', 'referers', 'history'].forEach(function(k) {
    var v = desc[k] || [];
    if (!Array.isArray(v)) {
      v = [v];
    }
...
function createAnnotation(desc) {
 ...
 // used to create a hierarchical position
  var position = [desc.annotatedBy], flattened = desc.type + unitSep;
  if (desc.annotatedBy === 'system' && desc.roots) {
    position = [desc.roots[0]];
  }
  if (desc.roots) {
    position = position.concat(desc.roots);
    // Keep this if it is being re-published.
