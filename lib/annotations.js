...
function createContentItem(desc) {
  ...
  ['annotations', 'visitors', 'referers', 'headers'].forEach(function(k) {
    var v = desc[k] || [];
    if (!Array.isArray(v)) {
      v = [v];
...
function createAnnotation(desc) {
  ...
  var anno = { type: desc.type, hasTarget: desc.hasTarget, annotatedAt : desc.annotatedAt || new Date().toISOString(), annotatedBy : desc.annotatedBy, validated: desc.validated || false};
 
 // used to create a hierarchical position 
  var position = [desc.annotatedBy];
  if (desc.roots) {
