...
var crlf = '\r\n', rsep = '  - ';
var specFields = {
  'PubmedArticle/Article/Language/' : 'LA',
  'PubmedArticle/DateCreated/' : 'PY',
  'PubmedArticle/Article/DOI/DOI/' : 'DO',
  'PubmedArticle/Author/' : 'AU',
  'PubmedArticle/Title/' : 'T1'
};
  ...
  results.forEach(function(hit) {
    ...
    res.annotations.forEach(function(anno) {
      ...
      if (!field) {
       ...
       var val = anno.flattened.replace(new RegExp('.*?' + annoLib.unitSep), '').replace(new RegExp(annoLib.unitSep, 'g'), '/');

       for (var s in specFields) {
         if (val.indexOf(s) === 0) {
           val = val.substr(s.length);
           field = specFields[s];
         }
       }
      ris.push(field + rsep + val);
    });

    ris.push('Y2' + rsep + res.timestamp);
    ris.push('ER' + rsep);
    ris.push('');
  });
